...
path_data_raw <- file.path(
  dir_data_raw, 
  "2021_09_16,Gela,email-Supplimental figure.xlsx"
)

data_tidy_mem_long <- purrr::map_df(sheet_vec, function(sheet) {
  data_raw <- readxl::read_xlsx(
    file.path(
      path_data_raw 
    ), 
    ...
    dplyr::select(`CCR7+CD45RA+`:`CCR7+CD45RA-`, `Sample ID`) %>%
    dplyr::mutate(
      pop = sheet,
      age = "adult",
      row = 1:(dplyr::n())
...
data_tidy_clin_infant_anele <- data_tidy_clin_infant_anele %>%
  ...
  dplyr::mutate(
    pid = sheet_to_r_pid_infant[pid_sheet]
  ) %>%
  dplyr::mutate(age = "infant") %>%
  dplyr::select(-pid_sheet)
```
...
```{r }
testthat::expect_identical(nrow(data_tidy_clin_infant_anele), 75L)
testthat::expect_identical(sum(data_tidy_clin_infant_anele$bcg == "bcg"), 50L)
testthat::expect_identical(unique(data_tidy_clin_infant_anele$race),
                           c("2", "1"))
testthat::expect_identical(unique(data_tidy_clin_infant_anele$sex),
                           c("2", "1"))
testthat::expect_identical(unique(data_tidy_clin_infant_anele$bcg),
                           c("bcg", "no bcg"))
```
...
```{r }
datautils::view_cols(
  data_tidy_clin_infant_anele
)
```

#### Joining infant clinical datasets

##### Checks

We join the above two datasets on `pid`, `race` and `bcg`. Since each `pid` between the datasets has the same `race` and `bcg` entry, we know that the joining is correct. 

```{r }
data_tidy_clin_infant <- data_tidy_clin_infant %>%
  dplyr::full_join(
    data_tidy_clin_infant_anele, 
    by = c("pid", "race", "bcg", "age")
  )
```
