...
data_tidy_mem_infant <- purrr::map_df(sheet_vec, function(sheet) {
...
data_tidy_mem_infant <- data_tidy_mem_infant %>%
  dplyr::mutate(
...
path_data_raw <- file.path(
  dir_data_raw, 
  "2021_10_01,Gela,email-Figure 6- Memory profile.xlsx"
)
data_tidy_mem_adult <- purrr::map_df(sheet_vec, function(sheet) {
  data_raw <- readxl::read_xlsx(
    file.path(
      path_data_raw 
    ), 
  ...
  data_raw %>%
    ...
    dplyr::mutate(
      day = stringr::str_remove_all(var, "Adult_Day_"), 
      day = stringr::str_remove_all(day, "CCR7") %>%
    ...
    dplyr::select(pid, age, day, pop, pop_sub, freq)
}) %>%
  tibble::as_tibble() %>%
  dplyr::filter(grepl("adult", pid))

data_tidy_mem_adult <- data_tidy_mem_adult %>%
  dplyr::mutate(
    pop_sub = purrr::map_chr(pop_sub, function(x) {
      switch(
        x, 
